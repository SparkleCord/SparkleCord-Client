class SparkleHook {
    static core = {
        ...
        parseWebhook(url) {
            const finalURL = url.replace(/(discordapp\.com|discord\.com)(\/api\/webhooks)/, "discord.com$2");
            if (!finalURL.match(/^https:\/\/discord\.com\/api\/webhooks\/\d+\/[\w-]+$/)) { console.error("Invalid webhook URL format!"); return url; }
            return finalURL;
        }
    }
    static logic = {
        async handleMessageSend({ messageData, webhookURL }) {
            if (messageData.automodStatus.containsBlockedContent) return;
            ...
            const attachments = await Promise.all((messageData.attachments || []).map(async a => {
                if (a.content.startsWith("data:image")) {
                    return { blob: await fetch(a.content).then(r => r.blob()), filename: a.name };
                }
