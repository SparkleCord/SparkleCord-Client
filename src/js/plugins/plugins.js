class EventBus {
    ...
    // Create a listener for an event, an ID can be used.
    on(event, callback, id = "") {
        if (!this.listeners[event]) {
            this.listeners[event] = {
                callbacks: [],    // Legacy array for backward compatibility
                ids: {}          // ID-based lookup { [id]: callback }
            };
        }
    ...
    // Call all listeners for a specific event.
    emit(event, data) {
        const listeners = this.listeners[event];
        if (!listeners) return;
        
        // Call all listeners (both ID-based and anonymous)
    ...
    off(event, id) {
        ...
        const callback = listeners.ids[id];
        listeners.callbacks = listeners.callbacks.filter(cb => cb !== callback);
    ...
    static plugins = [
        { 
            ...
            onEnable: () => {
                ...
                if (webhookURL) {
                    webhookURL = webhookURL.replace(/(discordapp\.com|discord\.com)(\/api\/webhooks)/, 'discord.com$2');
