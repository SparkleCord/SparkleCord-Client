...
class SettingsManager {
    static types = {
        ...
        "info-title": (opt) => `<p class="eyebrow" style="margin-bottom: -10px;">${opt.label}</p>`,
        ...
        // Text Input
        "text": (opt) => opt.multiline
            ? `${opt.label ? `<label for="${opt.id}">${opt.label}</label>` : ''}<textarea class="settings-textarea" ${opt.placeholder ? `placeholder="${opt.placeholder}"` : ''} id="${opt.id}"></textarea>`
            : `${opt.label ? `<label for="${opt.id}">${opt.label}</label>` : ''}<input type="text" ${opt.placeholder ? `placeholder="${opt.placeholder}"` : ''} id="${opt.id}">`,
